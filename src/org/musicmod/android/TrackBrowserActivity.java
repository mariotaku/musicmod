...
public class TrackBrowserActivity extends ListActivity implements View.OnCreateContextMenuListener,
	...
	private String mAlbumId, mArtistId, mPlaylist, mGenre, mSortOrder;
		...
		if (mAdapter == null) {
			...
			mAdapter = new TrackListAdapter(
					getApplication(), // need to use application context to
										// avoid leaks
					this,
					mEditMode ? R.layout.track_list_item_edit_mode : R.layout.track_list_item,
					null, // cursor
	...
	public void init(Cursor newCursor, boolean isLimited) {
		...
		if ("nowplaying".equals(mPlaylist)) {
			try {
				...
				mNowPlayingListener.onReceive(this, new Intent(BROADCAST_META_CHANGED));
			...
			mTrackListListener.onReceive(this, new Intent(BROADCAST_META_CHANGED));
		}
	}
		...
		if (mAlbumId != null) {
			...
			if (numresults > 0) {
				...
				String where = MediaStore.Audio.Media.ALBUM_ID
						...
						+ "="
						+ mTrackCursor.getLong(mTrackCursor
			...
			if (mPlaylist.equals("nowplaying")) {
				if (MusicUtils.getCurrentShuffleMode() == MusicPlaybackService.SHUFFLE_AUTO) {
				...
				Cursor cursor = MusicUtils.query(
						...
						ContentUris.withAppendedId(Playlists.EXTERNAL_CONTENT_URI,
								Long.valueOf(mPlaylist)), cols, null, null, null);
				if (cursor != null) {
	...
	private BroadcastReceiver mNowPlayingListener = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (intent.getAction().equals(BROADCAST_META_CHANGED)) {
				getListView().invalidateViews();
			} else if (intent.getAction().equals(BROADCAST_QUEUE_CHANGED)) {
				if (mDeletedOneRow) {
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		getMenuInflater().inflate(R.menu.options_menu_track, menu);
	...
	private Cursor getTrackCursor(TrackListAdapter.TrackQueryHandler queryhandler, String filter,
		...
		if (mGenre != null) {
			...
			if (mPlaylist.equals("nowplaying")) {
				...
				if (!TextUtils.isEmpty(filter)) {
					uri = uri.buildUpon().appendQueryParameter("filter", Uri.encode(filter))
							.build();
				}
				...
				ret = queryhandler.doQuery(uri, mPlaylistMemberCols, where.toString(), null,
						mSortOrder, async);
			}
		} else {
			if (mAlbumId != null) {
	...
	private class NowPlayingCursor extends AbstractCursor {
		...
		public NowPlayingCursor(IMusicPlaybackService service, String[] cols) {
	...
	static class TrackListAdapter extends SimpleCursorAdapter implements SectionIndexer {
		...
		private final String mUnknownArtist, mUnknownAlbum;
		...
		class TrackQueryHandler extends AsyncQueryHandler {
			...
			public Cursor doQuery(Uri uri, String[] projection, String selection,
				...
				return MusicUtils.query(mActivity, uri, projection, selection, selectionArgs,
						orderBy);
			}
		...
		private void getColumnIndices(Cursor cursor) {
			...
			if (cursor != null) {
				...
				if (mIndexer != null) {
					...
					mIndexer = new AlphabetIndexer(cursor, mTitleIdx, alpha);
				}
			}
		}
		...
		public View newView(Context context, Cursor cursor, ViewGroup parent) {
			...
			vh.mTrackName = (TextView) v.findViewById(R.id.track_name);
			vh.mArtistName = (TextView) v.findViewById(R.id.artist_name);
			vh.mTrackDuration = (TextView) v.findViewById(R.id.track_duration);
		...
		public void bindView(View view, Context context, Cursor cursor) {
			...
			vh.mTrackName.setText(vh.buffer1.data, 0, vh.buffer1.sizeCopied);

			int secs = cursor.getInt(mDurationIdx) / 1000;
			if (secs == 0) {
				vh.mTrackDuration.setText("");
			} else {
				vh.mTrackDuration.setText(MusicUtils.makeTimeString(context, secs));
			}
			...
			vh.mArtistName.setText(vh.buffer2, 0, len);
